...
TEST_F(AddFieldsTest, ShouldErrorOnNonObjectSpec) {
    ...
    BSONObj spec = BSON("$addFields"
                        << "foo");
    BSONElement specElement = spec.firstElement();
    ASSERT_THROWS_CODE(DocumentSourceAddFields::createFromBson(specElement, ctx()), UserException,
        40232);
};

// Verify that mutiple documents can be processes in a row with the addFields stage.
