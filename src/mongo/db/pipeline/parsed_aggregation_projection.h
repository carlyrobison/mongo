...
public:
    /**
     * Returns a Status -- with an error number and message if the specification is invalid,
     * or Status::OK() if the spec is valid for a projection.
     */
...
private:
    ...
    /**
     * Sets the _status to the correct error if an invalid projection specification is detected.
     */
    Status validate();
