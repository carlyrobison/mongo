...
//    assert.commandFailed(viewsDB.runCommand( { create: "badPipeline1", viewOn: "collection", pipeline: { x: 1 } } ));
//    assert.commandFailed(viewsDB.runCommand( { create: "badPipeline2", viewOn: "collection", pipeline: [ { $limit: 1 }, { $indexStats: {} } ] } ));
    ...
    assert(arrayEq(res.cursor.firstBatch, [{ name: "view", options: { viewOn: "collection", pipeline: pipe } }]));

    // Create a view on a non-existent collection.
    assert.commandWorked(viewsDB.runCommand( { create: "viewOnEmpty", viewOn: "nonexistent", pipeline: pipe } ));
    ...
    // assert.commandWorked(viewsDB.runCommand( { create: "viewCycle1", viewOn: "viewCycle2", pipeline: pipe } ));
    // assert.commandFailed(viewsDB.runCommand( { create: "viewCycle2", viewOn: "viewCycle1", pipeline: pipe } ));

    // Cannot create a view on reserved system collections (e.g. the oplog).
    // assert.commandFailed(viewsDB.runCommand( { create: "viewOnOplog", viewOn: "local.oplog.rs", pipeline: pipe } ));

    // View names are constrained to the same limitations as collection names.
    assert.commandFailed(viewsDB.runCommand( { create: "", viewOn: "emptyStringViewName", pipeline: pipe } ));
    assert.commandFailed(viewsDB.runCommand( { create: "system.local.new", viewOn: "collection", pipeline: pipe } ));
    assert.commandFailed(viewsDB.runCommand( { create: "dollar$", viewOn: "collection", pipeline: pipe } ));
}());
